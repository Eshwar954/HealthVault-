...
const UserFileBoard = () => {
  ...
  useEffect(() => {
    ...
    if (filterType) updated = updated.filter((file) => file.fileType === filterType);
    if (filterDoctor) updated = updated.filter((file) => file.doctorName === filterDoctor);
    if (filterReport) updated = updated.filter((file) => file.reportType === filterReport);
    if (sortOrder === "newest") {
    ...
    <div className="file-board-container">
      ...
      <div className="file-controls">
        ...
        <div className="filter-row">
          ...
          <select value={filterDoctor} onChange={(e) => setFilterDoctor(e.target.value)}>
            ...
            {[...new Set(files.map((file) => file.doctorName).filter(Boolean))].map((doctor) => (
          ...
          <select value={filterReport} onChange={(e) => setFilterReport(e.target.value)}>
            ...
            {[...new Set(files.map((file) => file.reportType).filter(Boolean))].map((report) => (
      ...
      {loading ? (
        ...
        <div className="empty-state">
          {files.length === 0 ? "No medical records found in your vault." : "No records match your filters."}
        </div>
      ) : (
